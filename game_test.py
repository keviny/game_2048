...
class Game2048(Widget):
    ...
    def on_key_down(self, window, key, *args):
        moved = False
        if key == 273:
            moved = self.move_topdown(top=True, from_keyboard=True)
        elif key == 274:
            moved = self.move_topdown(top=False, from_keyboard=True)
        elif key == 276:
            moved = self.move_leftright(right=False, from_keyboard=True)
        elif key == 275:
            moved = self.move_leftright(right=True, from_keyboard=True)
...
class Game2048App(App):
    ...
    params_dict = model.create_game_params_dict()
    target_board_pl = tf.placeholder(tf.float32, shape=(1, 4, 4, 1))
    saver = tf.train.Saver()
    sess = tf.Session()
    target_board_q, _, _ = model.generate_game_q(target_board_pl, params_dict, 1, 'evaluate_board')
    ...
    def build(self):
        ...
        if platform == 'android':
            ...
            Clock.schedule_interval(self.ai_move, .1)

        Game2048App.saver.restore(Game2048App.sess,  sys.argv[1])
    ...
    def ai_move(self, *args):
        value = Game2048App.sess.run(
            [Game2048App.target_board_q],
            feed_dict={
                Game2048App.target_board_pl: np.array(self.root.ids.game.get_board()).reshape(1, 4, 4, 1).astype(float)})
        print self.root.ids.game.get_board(), value
        Game2048App.internal_game.load_board(self.root.ids.game.get_board())
        raw_directions = Game2048App.internal_game.get_valid_directions(False)
        board_score_dict = Game2048App.internal_game.get_next_boards(raw_directions)

        valid_directions = filter(lambda x: x != -1, raw_directions)
        print "inspect:", board_score_dict, valid_directions, raw_directions
        next_total_score = []
        for direction in [game.Direction.left,
            ...
            board_q_value = Game2048App.sess.run(
                [Game2048App.target_board_q],
                feed_dict={Game2048App.target_board_pl: np.array(move_board).reshape(1, 4, 4, 1).astype(float)})
            next_total_score.append(board_q_value[0][0][0] + move_score)
        target_move = np.argmax(np.array(next_total_score))
        print next_total_score, target_move, max(next_total_score)
        self.root.ids.game.ai_move(target_move)
